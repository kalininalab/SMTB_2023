...
for dataset in train validation test; do
    for model in "${!models[@]}"; do
        ...
        esm-extract "${models[$model]}" "/shared/fluorescence/$dataset.fasta" "/shared/fluorescence/${models[$model]}/$dataset" --repr_layers $layers --include mean --toks_per_batch 550
    done
done